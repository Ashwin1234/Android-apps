...
public class MinMax {
	...
	private ArrayList<Integer> scoreList;//a list to contain all possible moves for X

	HashMap<Integer,Integer> state_minmax;
	...
	MinMax(String[] init)		
	{
		...
		state_minmax=print_minMax(stateList);				//print out min max for initial board states
	 ...
	 * @return
	 */
	HashMap<Integer, Integer> findMoves()
	{		
		...
		printList(stateList);
		state_minmax=print_minMax(stateList);
		return state_minmax;
	}
	 ...
	 * @param list
	 */
	HashMap<Integer,Integer> print_minMax(ArrayList<Node> list)
	{
		ArrayList<Integer> arr=new ArrayList<>();
		for(int x = 0; x < list.size(); x++ )
		{
			Node temp = list.get(x);
			arr.add(temp.getMinMax());
			if(temp.getMinMax()==10 || temp.getMinMax()==0) {
				state_minmax.put(list.get(x).getMovedTo(), temp.getMinMax());
			}
			System.out.print("state " + list.get(x).getMovedTo() + ": minMax: "+temp.getMinMax()+ "\n");
		}
